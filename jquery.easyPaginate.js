    ...
    $.fn.easyPaginate = function(option) {
        ...
        return this.each(function() {
            ...
            function init(page) {
                ...
                console.log(endPage);
                if (option.edges < startPage  && startPage - option.edges > '1') {
                    for (var n = 1; n <= option.edges; n++) {
                    ...
                    p.push('<a class="dots disabled">...</a>');
                } else if (startPage - option.edges == '1') {
                   for (var n = 1; n <= option.edges; n++) {
                        p.push('<a class="btn ' + (option.currentPage == n ? 'btn-primary disabled current' : "") + '" rel="' + n + '">' + n + '</a>');
                        //p.push(['<a class="p_btn"><span class="inner_btn"><span class="label '+className+'" onClick="_getData('+n+')" >'+n+'</span></span></a>'].join(""));
                    }
                }else if(startPage - option.edges == '0'){
                    for (var n = 1; n <= option.edges-1; n++) {
                        p.push('<a class="btn ' + (option.currentPage == n ? 'btn-primary disabled current' : "") + '" rel="' + n + '">' + n + '</a>');
                        //p.push(['<a class="p_btn"><span class="inner_btn"><span class="label '+className+'" onClick="_getData('+n+')" >'+n+'</span></span></a>'].join(""));
                    }
                }
                ...
                if (endPage < option.pages && option.pages - endPage != '1') {
                    p.push('<a class="dots disabled">...</a>');
                    for (var n = option.pages-option.edges+1; n <= option.pages; n++) {
                    p.push('<a class="btn ' + (option.currentPage == n ? 'btn-primary disabled current' : "") + '" rel="' + n + '">' + n + '</a>');
                    //p.push(['<a class="p_btn"><span class="inner_btn"><span class="label '+className+'" onClick="_getData('+n+')" >'+n+'</span></span></a>'].join(""));
                }
